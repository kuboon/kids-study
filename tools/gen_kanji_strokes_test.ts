...
import { pathEndpoints, svgToStrokes } from "./gen_kanji_strokes.ts";
...
Deno.test("svgToStrokes: extracts paths in order, ignores StrokeNumbers", () => {
  const svg = `<svg><g id="kvg:StrokePaths_x">
    <path id="kvg:x-s1" kvg:type="㇑" d="M50,10 L50,90"/>
    <path id="kvg:x-s2" kvg:type="㇐" d="M10,50 L90,50"/>
  </g><g id="kvg:StrokeNumbers_x"><text>1</text><text>2</text></g></svg>`;
  assertEquals(svgToStrokes(svg), [2, 0]); // vertical(S=2), horizontal(E=0)
});
