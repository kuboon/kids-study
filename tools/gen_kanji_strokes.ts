...
const OUT = new URL("../quiz/stroke/kanji_strokes.ts", import.meta.url);
const RAW_BASE =
...
// ---- SVG path endpoint walker ---------------------------------------------

type Pt = { x: number; y: number };

const TOKEN_RE = /[MmLlHhVvCcSsQqTtAaZz]|-?\d*\.?\d+(?:[eE][+-]?\d+)?/g;

/** Walk a path `d`, returning the first anchor and the final current point. */
...
const strokeDir = (d: string, type: string | undefined): number => {
  const { start, end } = pathEndpoints(d);
  const dx = end.x - start.x, dy = end.y - start.y;
  if (Math.hypot(dx, dy) < TINY_LEN) return firstType(type) ?? DEFAULT_FALLBACK;
  return quantize8(dx, dy);
};

/** Extract ordered stroke directions from one KanjiVG SVG. */
export const svgToStrokes = (svg: string): number[] => {
...
const main = async () => {
  ...
  for (const ch of chars) {
    ...
    result[ch] = strokes;
    console.log(
      `${ch} ${codeToFile(ch)}: [${strokes.join(",")}] ` +
        strokes.map((i) => DIR_ARROWS[i]).join(""),
    );
  }
  if (missing.length) {
    console.warn(
  ...
  if (dry) {
    console.log(
      `\n(dry run, ${Object.keys(result).length} chars, not written)`,
    );
    return;
  }
  ...
  const lines = keys.map((ch) => {
    const arr = result[ch];
    const arrows = arr.map((i) => DIR_ARROWS[i]).join("");
    return `  ${JSON.stringify(ch)}: [${arr.join(", ")}], // ${arrows}`;
  });
  const body = `// AUTO-GENERATED by tools/gen_kanji_strokes.ts. DO NOT EDIT.
// Stroke direction data derived from KanjiVG (https://kanjivg.tagaini.net),
// (C) Ulrich Apel, licensed CC BY-SA 3.0. This derived data is likewise
// CC BY-SA 3.0. See /NOTICE.
//
// Each value is a list of 8-direction codes (see quiz/stroke/dir.ts) in
// writing order: 0=E 1=SE 2=S 3=SW 4=W 5=NW 6=N 7=NE.

export const KANJI_STROKES: Record<string, readonly number[]> = {
${lines.join("\n")}
};
`;
  await Deno.writeTextFile(OUT, body);
  console.log(`\nwrote ${keys.length} chars → ${OUT.pathname}`);
};
